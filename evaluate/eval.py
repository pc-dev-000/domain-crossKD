...
class AnalysisThread_2D(QThread):
	update = pyqtSignal(object, object, object, object, object, object, object, object, object, object, object, object, object, object)
	end = pyqtSignal()
	...
	def run(self):
		...
		for k in range(self.class_num):
			...
			if(GT[k] == 0):
				...
				self.lFPR[k] = FP[k] / (TP[k]+FP[k]);
				self.lFNR[k] = FN[k] / (TP[k]+FN[k]);
		...
		self.update.emit(GT, TP, FP, FN, self.lACC, self.lFPR, self.lFNR, self.gACC, 0, self.gFPR, 0, self.gFNR, 0, self.ERHist)
		self.end.emit()
...
class AnalysisDataSetThread_2D(QThread):
	...
	def _setresulttext(self):
		...
		for k in range(self.class_num):
			...
			if(self.GT[k] == 0):
				...
				self.lFPR[k] = self.FP[k] / (self.TP[k] + self.FP[k]);
				self.lFNR[k] = self.FN[k] / (self.TP[k] + self.FN[k]);
		...
		self.wgFPR = sum(self.weighted_parameters*self.lFPR)
		self.wgFNR = sum(self.weighted_parameters*self.lFNR)
		...
		self.wgFNR /= constant
		if (self.gSum == 0):
			self.gACC /= (self.gSum + 1)
			self.gFNR /= (self.gSum + 1)
		else:
			self.gACC /= self.gSum
			self.gFNR /= self.gSum
		#self.gACC /= self.gSum
	...
	def run(self):
		...
		self._setresulttext()
		self.update.emit(self.GT, self.TP, self.FP, self.FN, self.lACC, self.lFPR, self.lFNR, self.gACC,self.wgACC, self.gFPR, self.wgFPR, self.gFNR, self.wgFNR, self.ERHist)
		self.end.emit(True)
...
class ToolUI(qws.QWidget):
	def __init__(self, parent=None):
		...
		self.Precision_Button = self.SetButton([a + (2 * ( s + 10)), 280], "FPR", "Precision_Button", size=[s, 30])
		self.Precision_Button.clicked.connect(self.draw)
		
		self.Recall_Button = self.SetButton([a + (3 * ( s + 10)), 280], "FNR", "Recall_Button", size=[s, 30])
		self.Recall_Button.clicked.connect(self.draw)
	...
	def _AnalysisThreadUpdate(self, GT, TP, FP, FN, lACC, lFPR, lFNR, gACC, wgACC, gFPR, wgFPR, gFNR, wgFNR, ERHist):
		self.GT = GT
		self.TP = TP
		self.FP = FP
		self.FN = FN
		...
		print("ACC  -  PRC  -  RCL")
		for k in range(self.class_num):
			print(self.lACC[k], '    ', 1 - self.lFPR[k], '    ', 1 - self.lFNR[k])
		print("\n")
	...
	def _SetResultText(self):
		...
		print("ACC  -  wACC  -  PRC -  wPRC  -  RCL -  wRCL")
		print(self.gACC, '   ', self.wgACC, '   ', 1 - self.gFPR, '   ', 1 - self.wgFPR, '   ', 1 - self.gFNR, '   ', 1 - self.wgFNR)
		print("\n")
		data_need_to_show = "{0:^20} = {1:2.4f}\n".format("Accuracy", self.gACC)
		data_need_to_show += "{0:^20} = {1:2.4f}\n".format("Weighted Accuracy", self.wgACC)
		data_need_to_show += "{0:^20} = {1:2.4f}\n".format("Precision", 1 - self.gFPR)
		data_need_to_show += "{0:^20} = {1:2.4f}\n".format("Weighted Precision", 1 - self.wgFPR)
		data_need_to_show += "{0:^20} = {1:2.4f}\n".format("Recall", 1 - self.gFNR)
		data_need_to_show += "{0:^20} = {1:2.4f}\n".format("Weighted Recall", 1 - self.wgFNR)
	...
	def draw(self):
		...
		elif(s.objectName() == "Precision_Button"):
			self.Fig.bar(title = "Precision", x_data = self.short_header_class, y_data = self.lFPR)
		elif(s.objectName() == "Recall_Button"):
			self.Fig.bar(title = "Recall", x_data = self.short_header_class, y_data = self.lFNR)
	...
	def Export(self):
		...
		if fileName:
			...
			table.write(2, 0, "ALL CLASSES Weighted Accuracy")
			table.write(2, 1, self.wgACC)
			table = self._write(file, 'Class Precision', 1 - self.lFPR)
			table.write(2, 0, "ALL CLASSES Weighted Precision")
			table.write(2, 1, 1 - self.wgFPR)
			table = self._write(file, 'Class Recall', 1 - self.lFNR)
			table.write(2, 0, "ALL CLASSES Weighted Recall")
			table.write(2, 1, 1 - self.wgFNR)
