...
def show4itri(image, input_image, class_index, class_name, input_bbox):
    ...
    cv2.putText(input_image, class_name, (x1-10, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
...
def save_annotations(name, input_image, c_index, c_boxes, fov_indices, total_detection):
    ...
    out_txt_path = file_path.replace("RGB", "TXT_TEACHER")
    out_vis_path = file_path.replace("RGB", "VIS_TEACHER")
